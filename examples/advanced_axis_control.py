...
async def main():
    ...
    try:
        ...
        logger.info(
            f"Microsteps per revolution for pulse commands is now: {axis._microsteps_per_motor_revolution_for_cmd}"
        )
        ...
        logger.info(
            f"Motor status after E-Stop: {const.MOTOR_STATUS_MAP.get(status_code, 'Unknown')}"
        )
        ...
        if axis is not None and axis.is_enabled():
